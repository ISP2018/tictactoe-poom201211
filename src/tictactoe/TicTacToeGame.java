...
/**
 * This class is for initializing the board and the function of
 * the game. For example, initializing the board and the X,O pieces.
 *
...
public class TicTacToeGame {
	...
	public Player winner() {
		...
		int decrease = boardsize-1;
		int countTwo = 0;
		for(int k = 1; k < boardsize; k++){
			decrease--;
			if (p != Player.NONE && p == pieces[k][decrease].type) {
				countTwo++;
				// all pieces on diagonal occupied by same type (Player)
				if(countTwo == boardsize-1){
					return p;
				}
			}
		}
